...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
            test_data_train, test_data_test, mean=None, std=None, norm_reg=0):
        ...
        if mean is not None:
            for i in range(0, len(training_data)):
                ...
                training_data[i] = (tuple[0] + noise, tuple[1])
        accuracy_train = []
        accuracy_test = []
        ...
        for j in range(epochs):
            ...
            for mini_batch in mini_batches:
                self.update_mini_batch(mini_batch, eta, norm_reg)
    ...
    def update_mini_batch(self, mini_batch, eta, norm_reg=0):
        """Update the network's weights and biases by applying
        gradient descent using backpropagation to a single mini batch.
        The ``mini_batch`` is a list of tuples ``(x, y)``, and ``eta``
        is the learning rate."""
        ...
        for x, y in mini_batch:
            ...
            nabla_b = [nb+dnb for nb, dnb in zip(nabla_b, delta_nabla_b)]
            nabla_w = [nw+dnw for nw, dnw in zip(nabla_w, delta_nabla_w)]
        self.weights = [w-(eta/len(mini_batch))*nw - w*norm_reg for w, nw in zip(self.weights, nabla_w)]
